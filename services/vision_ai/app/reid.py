...
redis_stream_detections = db.Stream("detections")
...
def tiler_sink_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        modified_detections = save_crops_reid(original_frame, core_detections, str(frame_meta.pad_index))
        frame_copy = np.array(n_frame, copy=True, order='C')
        # convert the array into cv2 default color format
        frame_copy = cv2.cvtColor(frame_copy, cv2.COLOR_RGBA2BGRA)

        img_path = f"/tmpfs/{gv.folder_name}/stream_{frame_meta.pad_index}/frame_{frame_number}.jpg"
        cv2.imwrite(img_path, frame_copy)
        frame_dets = {}
        frame_dets["frame_no"] = frame_number
        frame_dets["camera_id"] = f"source_{frame_meta.pad_index}"
        frame_dets["detections"] = modified_detections
        frame_dets["img_path"] = img_path
        data_output.append(frame_dets)

        if save_image:
            img_path = f"{gv.folder_name}/stream_{frame_meta.pad_index}/frame_{frame_number}.jpg"
            cv2.imwrite(img_path, frame_copy)
    ...
    msg = {"batch": json.dumps(data_output)}
    redis_stream_detections.add(msg, maxlen=1000, approximate=True)
...
def main(videos=[], folder_name="frames"):
    ...
    for i, name in enumerate(videos):
        try:
            os.makedirs(f"{gv.folder_name}/stream_{str(i)}")
            os.makedirs(f"{gv.folder_name}/stream_crops_{str(i)}")
            os.makedirs(f"/tmpfs/{gv.folder_name}/stream_{str(i)}")
            os.makedirs(f"/tmpfs/{ gv.folder_name}/stream_crops_{str(i)}")
        except:
            pass
...
@app.post("/start")
def start_pipeline(reid_conf: float = Query(...), single_reid: bool = Query(...), multi_reid: bool = Query(...)):
    global pipeline_process
    try:
        gv.REID_CONF = reid_conf
        gv.SINGLE_REID = single_reid
        gv.MULTI_REID = multi_reid
    except:
        pass
    logger.info(f"STARTING PIPELING WITH FOLLOWING:\n"
            f"REID CONFIDENCE: {gv.REID_CONF}\n"\
            f"SINGLE CAMERA REID: {gv.SINGLE_REID}\n"\
            f"MULTI CAMERA REID:{gv.MULTI_REID}")
    base_path = "file://"
    if pipeline_process is None or not pipeline_process.is_alive():
        pipeline_process = Process(target=main, args=([base_path + str(settings.VIDEOS_DICT.get("video_1")), base_path + str(settings.VIDEOS_DICT.get("video_2"))], "frames"))
...
def stop_pipeline():
    ...
    if pipeline_process and pipeline_process.is_alive():
        pipeline_process.terminate()  # Terminate the process
        pipeline_process.join()       # Wait for the process to terminate
        return {"status": "Pipeline stopped"}
    return {"status": "Pipeline is not running"}